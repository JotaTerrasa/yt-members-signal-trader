...
flowchart TB
  ...
  subgraph trading["Trading y riesgo"]
    ...
    server --> matcher["ReplicaAuditMatcher<br/>hoja ↔ apertura ↔ cierre ↔ comisiones"]
    trader --> bingxClient["BingXClient<br/>REST firmado"]
    trader --> paper["PaperTradeStore<br/>prueba/simulación local"]
...
- abre/cierra la simulación local;
...
- enlaza PnL, comisión de apertura, comisión de cierre y financiación;
...
El frontend deriva de esas identidades una banda `Diagnóstico económico` sin introducir una contabilidad paralela. La brecha total se calcula como `bingxNet - replicaPnl`; ejecución y costes proceden de los pasos reconciliados, y cobertura/ausencias es el residual algebraico restante. Los porcentajes visuales usan solo magnitudes negativas para no atribuir como pérdida un tramo que compensa el resultado. Si existe una cohorte activa, el componente usa inicialmente `audit.cohort.summary` y permite cambiar a `audit.summary` para consultar el mes completo. El cambio de ámbito conserva la misma identidad contable y calcula de nuevo qué fase, entrada o salida, concentra el deterioro neto. El componente es analítico, adaptable a móvil y no participa en ninguna decisión de ejecución.
...
La misma respuesta incluye `source.orderHistory` y `summary.orderHistoryEvidence`. El primer bloque describe la lectura firmada de BingX; el segundo demuestra cuántos ciclos usan ejecuciones exactas, cuántas aperturas se recuperaron desde el exchange y si queda algún cierre sin apertura. El frontend identifica visualmente el histórico vigente, una copia obsoleta o el respaldo. Esta ruta es de solo lectura y no participa en la creación, modificación ni cierre de órdenes.
...
`operationalIncidents.js` agrupa en el servidor las incidencias repetidas antes de limitar la respuesta a 30 grupos. Conserva `firstAt`, `lastAt` y `occurrences`; el resumen distingue ocurrencias brutas, grupos totales y grupos mostrados. Los errores genéricos solo se fusionan cuando título y mensaje coinciden, por lo que dos fallos diferentes no quedan ocultos bajo el tipo común `error`. El frontend añade después el ciclo de vida visual. `trackTelegramVisibility` recuerda la última lectura con mensajes; una incidencia `telegram_web_empty` permanece recuperada durante el mayor valor entre 90 segundos y tres intervalos de recarga, de modo que el cero transitorio de `page.reload()` no produzca parpadeos. Una ausencia sostenida supera la gracia y vuelve a quedar activa. No se altera ninguna lectura ni decisión operativa.
...
El proceso PM2 no recibe el entorno completo de la consola que lo inicia. `ecosystem.config.cjs` filtra todas las variables heredadas y reconstruye una lista blanca con rutas del sistema, directorios temporales, localización, Playwright y las variables explícitas de servidor. `scripts/checkPm2Environment.js` inspecciona proceso y volcado persistente mediante nombres sensibles, sin serializar valores. Los arranques programados y la restauración tras el backup reaplican el ecosistema antes de ejecutar `pm2 save`.

La continuidad local tiene dos niveles diferenciados. El servidor genera un snapshot redactado para diagnóstico, mientras `scripts/secureBackup.js` crea el contenedor restaurable AES-256-GCM. Las tareas programadas usan `--drill`: vuelven a descifrar el contenedor publicado, validan que todas las rutas pertenezcan exclusivamente a `.data` o `.yt-profile`, lo extraen en un directorio temporal y comprueban sus raíces antes de limpiarlo. `status.json` conserva solo metadatos saneados y `/api/operational-status` los transforma mediante `backupContinuity.js`; la interfaz puede distinguir una copia meramente redactada de una copia cifrada cuya restauración se ha demostrado.

`backupStorage.js` inspecciona el sistema de archivos sin modificarlo. Cuenta únicamente contenedores `.fmbak` y parciales, calcula bytes libres mediante `statfs` y clasifica el volumen con umbrales absolutos y porcentuales. El servidor conserva la lectura durante un minuto, la renueva cada quince minutos y limita las alertas repetidas a una cada seis horas. Las rutas y los nombres de archivo no forman parte de la respuesta pública; tampoco existe ninguna eliminación o rotación automática.

La clave de cifrado tiene un flujo de continuidad separado. `export-key` materializa la clave efectiva en una ruta indicada expresamente, utiliza creación exclusiva, aplica permisos privados y compara una huella antes de confirmar la copia. `verify-key` vuelve a compararla sin publicar su contenido. El destino no puede pertenecer al proyecto, al directorio privado activo ni al mismo sistema de archivos que la réplica de contenedores; el estado público conserva únicamente una huella truncada, una etiqueta y la relación de volumen. Ninguna tarea automática exporta la clave.

La réplica externa es optativa y se configura fuera del repositorio. Después del simulacro local, el contenedor cifrado se copia a un nombre parcial en la ubicación elegida, se descifra y valida desde allí y se renombra de forma atómica. El proceso conserva siempre el backup local; una réplica fallida devuelve error al Programador de tareas y queda roja en su métrica independiente, sin degradar el estado de una copia local que ya se ha restaurado con éxito. La desactivación es reversible: cambia atómicamente el estado privado a `enabled: false`, actualiza la UI y no elimina la ruta configurada ni ningún contenedor. La comparación de dispositivo distingue otra unidad de una simple carpeta en el mismo volumen, y la API solo publica una etiqueta saneada, nunca la ruta completa.

Los cambios exclusivamente estáticos no renuevan `runtime.id`, por lo que el frontend consulta mediante `HEAD` los `ETag` de `index.html`, `app.js`, `styles.css` y `polish.css` una vez por minuto y al recuperar el foco. Si la huella cambia, muestra un aviso persistente y programa una recarga del documento a los cinco segundos. Un `input`, `textarea`, `select` o elemento editable con foco aplaza la recarga en ciclos de cinco segundos para no interrumpir datos que se estén escribiendo; `Actualizar ahora` mantiene la vía manual. La URL recibe una marca `ui-version` única para saltarse documentos antiguos. El monitor, el scraping y la ejecución permanecen en el backend. La prueba Chromium cambia los cuatro `ETag`, exige el aviso, verifica el aplazamiento durante la edición y cubre las recargas manual y automática sin errores JavaScript.

La vista de Rendimiento incorpora una navegación secundaria fija sobre ocho bloques estables. La hoja propia y la hoja externa son destinos distintos, aunque compartan la misma banda de trabajo. Todos los enlaces internos usan `data-pnl-anchor` y pasan por un único controlador que actualiza el historial, desplaza el destino y le transfiere el foco sin añadirlo al recorrido normal del tabulador. Un cálculo limitado mediante `requestAnimationFrame` observa el desplazamiento y mantiene `aria-current="location"` en la sección visible; en anchuras pequeñas también desplaza la propia banda para conservar el enlace activo a la vista. `.content-panel` usa `overflow: clip`: mantiene el recorte visual sin crear un contenedor de scroll que inutilice `position: sticky`. Esta capa solo organiza la interfaz y no interviene en parsing, tamaño, validación ni envío de órdenes.

El arranque del frontend no usa una barrera `Promise.all`: conecta SSE primero y resuelve estado, Telegram, Telegram Web, estudio, guardia, BingX y publicaciones de forma independiente. Cada lectura inicial dispone de 8 segundos; al vencerlos, `AbortController` cancela la petición para que `Promise.allSettled` pueda terminar. Un fallo deja un aviso de carga parcial, pero no cancela las demás fuentes ni el canal en tiempo real. Solo las cargas fallidas entran en un reintento con backoff de 2, 5, 15 y 30 segundos; el aviso desaparece cuando se recuperan. El render de auditoría admite explícitamente un valor nulo mientras `/api/replica-audit` continúa cargando, de modo que una respuesta asíncrona no puede abortar el resto de Rendimiento. `scripts/frontendBootstrapCheck.js` inyecta un `503` a `/api/telegram`, un payload SSE corrupto y una respuesta bloqueada de `/api/telegram-source` dentro de Chromium; los tres ciclos deben recuperarse en la puerta Docker. La misma comprobación sirve una auditoría económica determinista de seis operaciones y valida los dos ámbitos, totales, categorías, activos, scroll, filtros y enlaces a la evidencia sin consultar BingX ni Google Sheet.

La carga de Rendimiento aplica el mismo aislamiento a `/api/historical-pnl`, `/api/bingx/pnl-sources`, `/api/replica-audit` y `/api/bingx/pnl`, con un máximo de 30 segundos por fuente. Las cuatro peticiones comienzan en paralelo, pero el resultado histórico se aplica y renderiza en cuanto termina su propia promesa; la tabla no espera a que BingX o la auditoría resuelvan. `Promise.allSettled` permite completar después cada resultado disponible y conservar el último histórico válido. La lectura HTTP de `gviz` dispone de quince segundos; cuando vence o Google falla y existe una hoja en la caché de memoria, `loadReferenceLedger` devuelve esa última copia con metadatos `stale` y `warning` en vez de vaciar el panel. Si la primera lectura fría de `/api/historical-pnl` vence, `scheduleReferenceWarmRetry` actualiza solo la referencia con una espera progresiva de uno a treinta segundos hasta disponer de filas; cada intento del cliente admite 60 segundos y suele reutilizar la petición o caché que el servidor ya estaba preparando. El temporizador se cancela al recuperar la hoja y nunca participa en parsing, reintentos operativos ni órdenes. Por tanto, una caída de Google Sheet no impide actualizar VST/real, y un fallo de BingX no vacía la tabla externa. La tabla informa de la fecha de su última operación y calcula su retraso con un umbral de 24 horas; la hora de descarga queda solo como detalle. La actualización periódica conserva las cachés breves. `Actualizar` añade `refresh=1` a las cuatro lecturas, mientras que el control local consulta únicamente `/api/historical-pnl` y `/api/replica-audit` sin invalidar la caché disponible; además pinta el histórico antes de que termine la auditoría. El control local solo aparece cuando existe una fuente, refleja `aria-busy` mientras espera la hoja y no lanza una segunda lectura concurrente. `replicaAuditScopeContext` entrega un único resumen y un único conjunto de filas para el ámbito activo; todos los componentes analíticos consumen ese contexto, por lo que la cabecera no puede mostrar una cohorte mientras las tablas usan el mes completo. La navegación por ancla espera a que termine la carga de PnL, aplica dos fotogramas de estabilización y salta de forma instantánea aunque el documento use desplazamiento suave. Cada destino analítico usa `tabindex="-1"`: no añade paradas al recorrido normal, pero recibe el foco cuando el usuario activa su enlace. Si un renderizado sustituye el nodo y deja el foco en el documento, la intención se restaura sobre el nodo vigente; si el usuario ya eligió otro control, no se le roba el foco. Cada gráfico contable publica `plotly-layout-stable` al completar `Plotly.react`; si el usuario pulsó antes, el enlace vuelve a resolver una única vez el nodo vigente y corrige el desplazamiento causado por el cambio de altura. Un `ResizeObserver` y un `MutationObserver` temporales vigilan el alto y el contenido completos de Rendimiento durante la ventana de asentamiento, de modo que una gráfica situada antes del destino o la sustitución de un panel con idénticas dimensiones tampoco puedan apartarlo de la vista ni perder el foco. Los observadores se desconectan al estabilizarse la vista, al cambiar de ancla o, como máximo, a los seis segundos. Los renderizados completos posteriores vuelven a resolver el destino y recuperan el foco si este quedó en el documento; un evento `focusin` sobre cualquier otro control cancela la intención antes de que pueda interferir con la elección del usuario. La prueba Chromium retrasa deliberadamente la carga local de Plotly, activa los tres accesos con teclado, sustituye un destino con y sin el observador temporal, introduce un cambio artificial de 900 píxeles antes del panel y exige que ejecución, costes, cobertura y el ancla principal alcancen su evidencia con desplazamiento y foco correctos. También mantiene BingX y auditoría lentos para demostrar la pintura temprana de la hoja, fuerza un `503` de histórico, exige que las demás fuentes terminen, monta una hoja sintética para comprobar el reintento automático e impedir que vuelva el visor `iframe` bloqueable de Google, exige que el panel quede dentro de la vista y comprueba que el refresco local permanezca aislado de BingX. Además reproduce vistas de 390 por 844 y 768 por 1024 píxeles, y exige desplazamiento bidireccional dentro de la tabla, ausencia de desbordamiento global y una distribución legible de los cinco dominios de fiabilidad.
