...
La réplica externa es optativa y se configura fuera del repositorio. Después del simulacro local, el contenedor cifrado se copia a un nombre parcial en la ubicación elegida, se descifra y valida desde allí y se renombra de forma atómica. El proceso conserva siempre el backup local; una réplica fallida devuelve error para que el Programador de tareas y la UI lo detecten. La comparación de dispositivo distingue otra unidad de una simple carpeta en el mismo volumen, y la API solo publica una etiqueta saneada, nunca la ruta completa.
