...
El centro de incidentes aplica una agrupación exclusivamente visual por tipo y ciclo de vida. Los avisos repetidos conservan un contador y la marca temporal más reciente. `trackTelegramVisibility` recuerda la última lectura con mensajes; una incidencia `telegram_web_empty` permanece recuperada durante el mayor valor entre 90 segundos y tres intervalos de recarga, de modo que el cero transitorio de `page.reload()` no produzca parpadeos. Una ausencia sostenida supera la gracia y vuelve a quedar activa. No se altera ninguna lectura ni decisión operativa.
