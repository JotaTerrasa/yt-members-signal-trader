...
Los cambios exclusivamente estáticos no renuevan `runtime.id`, por lo que el frontend consulta mediante `HEAD` los `ETag` de `index.html`, `app.js`, `styles.css` y `polish.css` una vez por minuto y al recuperar el foco. Si la huella cambia, muestra un aviso persistente con `Actualizar interfaz`. La acción recarga únicamente el documento; el monitor, el scraping y la ejecución permanecen en el backend. La prueba Chromium cambia los cuatro `ETag`, exige el aviso, pulsa la acción y verifica que la página nueva vuelve a estado vigente sin errores JavaScript.
