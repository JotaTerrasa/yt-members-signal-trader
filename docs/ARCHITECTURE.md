...
Los eventos nuevos incorporan `executionTelemetry` con dos lecturas temporizadas de `lastPrice`, la instantánea `bookTicker` más próxima al envío y el RTT local de la petición de orden. La fila auditada lo expone como `vst.entryTelemetry`. A partir de esa evidencia, la auditoría separa spread, `lastPrice → ask` para LONG o `lastPrice → bid` para SHORT, y `precio ejecutable → fill`. `packageObservation` añade la hora de inicio del paquete, su tamaño, la posición exacta de la señal y el bid/ask disponible en ese instante. Esto permite medir `inicio del paquete → preenvío` como un tramo independiente. La cobertura es prospectiva: no reconstruye bid/ask históricos ni convierte su ausencia en cero.
