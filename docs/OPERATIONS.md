...
- En Windows, `FuturesMagicianPM2Startup` es la única vía de autoarranque. `npm run windows:tasks` retira el acceso directo heredado de la carpeta Inicio y `startPm2.ps1` impide que dos restauraciones simultáneas compitan por el puerto `5178`.
...
- El botón `Actualizar` fuerza una lectura nueva de `/api/historical-pnl`, `/api/bingx/pnl-sources`, `/api/replica-audit` y `/api/bingx/pnl` mediante `refresh=1`. Omite las cachés locales de cinco minutos y 45 segundos, pero respeta el cooldown de BingX para no insistir durante un bloqueo temporal. Dos lecturas concurrentes de la misma hoja se deduplican y ninguna de estas rutas ejecuta operaciones.
...
- `Incidencias 24h` conserva el historial, pero contrasta cada aviso con el estado actual. Una reconciliación de BingX, un feed de YouTube, el monitor o un cooldown de PnL que ya estén sanos aparecen como `Recuperada`; los backups y mensajes de servicio figuran como `Registro`; solo `Activa` influye en el contador de avisos vigente. Esta clasificación es visual y no modifica reintentos, órdenes ni alertas del backend.
...
- Rendimiento concede 30 segundos por lectura a Google Sheet, auditoría, fuentes VST/real y PnL de cuenta. Los resultados se aplican por separado: si falla la hoja, sus filas conservadas siguen visibles con un aviso y BingX continúa; si falla BingX, la hoja y la auditoría no se descartan. La puerta Chromium incluye una caída forzada de `/api/historical-pnl`, comprueba que las tarjetas de fuentes terminan de renderizarse y valida con datos sintéticos que la hoja use la tabla nativa y muestre su cobertura temporal.
...
- `pairedOutcomeImpact` cuantifica esas categorías sobre la misma muestra y en VST. Para el total, cada causa y cada activo publica réplica escalada, bruto de BingX, brecha bruta antes de costes, comisiones, funding, neto, brecha neta frente a la réplica y residual. `costShareOfGapPercent` solo se calcula cuando brecha bruta y costes empeoran el resultado en la misma dirección; si se compensan, permanece nulo para no presentar un porcentaje engañoso. Las categorías primarias forman una partición completa de la muestra, por lo que no se suman dos veces los casos cuyo bruto volvió a alinearse después de costes. Un residual superior a 0,01 VST se considera una incidencia crítica del informe. Este bloque es exclusivamente analítico y no participa en aperturas, cierres ni validaciones operativas.
...
- `Diagnóstico económico` sintetiza ese puente antes del detalle. La barra distribuye únicamente las contribuciones negativas entre ejecución emparejada, costes y cobertura/ausencias; una contribución positiva no se presenta como arrastre. La conclusión destaca la causa dominante, el peso de las salidas dentro de la ejecución y el mayor tramo de `executionPriceChain`. La nota de evidencia recuerda que el mes completo incluye incidencias históricas separadas y que la cohorte vigente debe evaluarse por su propio bloque.
