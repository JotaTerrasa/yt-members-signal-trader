...
- La banda fija de Rendimiento permite saltar entre `Resumen`, `Fiabilidad`, `Guardia`, `Posiciones`, `Mi hoja`, `Rendimiento` y `Auditoría`. El enlace activo sigue el bloque visible y usa `aria-current`; al activar un destino con teclado, este recibe el foco sin añadir una parada permanente al tabulador. En móvil la banda conserva una sola línea con desplazamiento horizontal propio. La prueba Chromium exige que las barras fijas no se solapen, que el destino quede visible y que la página no adquiera scroll horizontal.
