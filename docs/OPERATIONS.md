...
- El subpanel `Dónde se deterioran las entradas` separa `señal → cotización previa` de `cotización previa → fill`, y agrupa la cohorte por activo, ruta, latencia y franja horaria de Madrid. Una comparación necesita al menos tres aperturas en cada cohorte; con menos evidencia se marca como insuficiente.
- Este diagnóstico es estrictamente informativo. No bloquea activos u horarios, no cambia el umbral de entrada, no reordena reintentos y no modifica ninguna orden. Cualquier cambio operativo posterior exige una hipótesis explícita y una muestra controlada nueva.
