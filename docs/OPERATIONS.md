...
- La sección `Hoja externa en vivo` usa la misma lectura `gviz` que alimenta la auditoría y la muestra como tabla local desplazable. No inserta un `iframe` de Google; `Hoja externa` abre el documento original en otra pestaña. El encabezado muestra la fecha de la última operación disponible, añade el retraso al superar 24 horas y conserva la hora de lectura local en el texto emergente. Las filas sin PnL se muestran como abiertas, con el segundo precio identificado como SL, y solo los cierres forman el win rate. `Mi hoja operativa`, la hoja externa y la auditoría cargan sus filas de forma incremental para mantener fluida la vista; sus resúmenes siempre se calculan con la muestra completa.
