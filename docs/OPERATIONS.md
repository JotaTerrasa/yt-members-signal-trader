...
- Mientras la vista PnL permanece visible, la hoja y `GET /api/replica-audit` se actualizan cada cinco minutos. La carga se pausa con la pestaña oculta, nunca se solapa con una actualización manual y conserva el último dato válido ante un fallo. El ciclo periódico aplica una espera exponencial de hasta treinta minutos. Si el primer arranque todavía no dispone de ninguna fila, la recuperación en frío reintenta con esperas progresivas de uno a treinta segundos; cada petición caduca a los sesenta segundos. La auditoría puede leer el histórico firmado de Demo VST, pero ninguna de estas rutas crea, modifica o cierra órdenes en BingX.
- El botón general `Actualizar` fuerza una lectura nueva de `/api/historical-pnl`, `/api/bingx/pnl-sources`, `/api/replica-audit` y `/api/bingx/pnl` mediante `refresh=1`. El icono situado junto a `Hoja externa` es una recuperación rápida y aislada: consulta únicamente `/api/historical-pnl` y `/api/replica-audit`, admite la caché breve disponible y muestra la tabla en cuanto llega el histórico, sin esperar a la auditoría. El icono local solo aparece cuando existe una hoja configurada, se deshabilita mientras espera el histórico y refleja la carga en su etiqueta accesible. Dos lecturas concurrentes de la misma hoja se deduplican y ninguna de estas rutas ejecuta operaciones.
- Cada lectura HTTP de Google Sheet se cancela a los quince segundos. Si ya existe una lectura correcta en memoria, el servidor devuelve esa última copia válida con un aviso de obsolescencia en vez de retirar las filas del panel; una actualización posterior reemplaza la copia automáticamente.
