...
- Si solo cambian los recursos del frontend, la pestaña comprueba sus `ETag` cada minuto y al recuperar el foco. El aviso `Nueva versión disponible` permite recargar HTML, JavaScript y estilos sin reiniciar ni detener el monitor. `npm run ui:recovery:check` simula el cambio y verifica el ciclo completo hasta que la interfaz vuelve a estar vigente.
