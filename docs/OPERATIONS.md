...
- `pairedOutcomeImpact` cuantifica esas categorías sobre la misma muestra y en VST. Para el total, cada causa y cada activo publica réplica escalada, bruto de BingX, comisiones, funding, neto, brecha frente a la réplica y residual. Las categorías primarias forman una partición completa de la muestra, por lo que no se suman dos veces los casos cuyo bruto volvió a alinearse después de costes. Un residual superior a 0,01 VST se considera una incidencia crítica del informe. Este bloque es exclusivamente analítico y no participa en aperturas, cierres ni validaciones operativas.
