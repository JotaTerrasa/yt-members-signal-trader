...
- Si una consulta inicial falla, la cabecera y SSE siguen activos y la UI muestra `Panel cargado parcialmente`. La aplicación reintenta exclusivamente las fuentes fallidas hasta recuperarlas. Si un evento SSE contiene JSON truncado, descarta solo ese evento, conserva el último estado válido y limpia el aviso con el siguiente payload correcto o heartbeat. `npm run ui:recovery:check` reproduce en Chromium tanto el fallo temporal de Telegram como un evento SSE corrupto; `npm run docker:check` ejecuta ambos casos dentro de la imagen, además de comprobar health, streaming y persistencia.
