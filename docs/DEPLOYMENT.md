...
El registro crea tres tareas: restauración de PM2 al iniciar sesión, backup cifrado diario a las 03:15 y backup semanal del perfil Chromium los domingos a las 04:00. Las tres reintentan hasta tres veces, con dos minutos entre intentos, cuando el proceso termina con error. Ambas tareas de backup descifran y extraen cada copia en un directorio temporal para demostrar que es restaurable; el estado saneado aparece en la guardia nocturna. La copia semanal confirma la parada de PM2 antes de capturar el perfil y reutiliza el arranque endurecido en su bloque `finally`, también cuando falla el backup. El arranque ejecuta `pm2 resurrect`, reaplica siempre `ecosystem.config.cjs` con `startOrReload --update-env`, espera hasta 90 segundos a que `/api/health` devuelva `ok` y un identificador de runtime, y solo entonces guarda de nuevo el estado. El ecosistema descarta el entorno heredado y repone solo rutas del sistema y variables explícitas de la app, evitando que credenciales de otras herramientas queden en el proceso o en `dump.pm2`. `FuturesMagicianPM2Startup` debe ser la única vía de autoarranque en Windows; el registro elimina el acceso directo heredado `yt-members-signal-trader-pm2-resurrect.lnk` y el script serializa invocaciones simultáneas para evitar carreras y errores `EADDRINUSE`.
