...
- El centro de incidentes agrupa en el backend los avisos repetidos antes de aplicar el límite de filas, conserva su frecuencia y evita que una fuente ruidosa oculte fallos diferentes. Una lectura vacía de Telegram Web pasa a `Recuperada` cuando vuelven a encontrarse mensajes y conserva una gracia breve durante las recargas normales, evitando presentar como activas interrupciones transitorias ya resueltas.
...
- Comprueba una vez por minuto la versión de HTML, JavaScript y estilos. Si cambian sin reiniciar el backend, muestra un aviso y recarga automáticamente la interfaz cuando no se está editando un campo; el botón `Actualizar ahora` permite hacerlo al instante. El monitor y las órdenes continúan en el servidor mientras se recarga la página.
...
- Carga Google Sheet, auditoría de réplica, fuentes VST/real y PnL de cuenta de forma independiente, con 30 segundos máximos por lectura. La tabla de la hoja se pinta en cuanto llega su propia respuesta, aunque BingX o la auditoría sigan cargando. Si la primera lectura fría de la hoja vence, realiza un único reintento inmediato para aprovechar la caché que el servidor termina de preparar. Una caída de la hoja no bloquea las cifras de BingX y una caída de BingX no elimina la última copia válida de la hoja.
...
Ese comando registra el arranque de PM2, el backup cifrado diario a las 03:15 y el backup semanal del perfil los domingos a las 04:00. Las tres tareas reintentan hasta tres veces, con dos minutos entre intentos, si terminan con error. La tarea `FuturesMagicianPM2Startup` es la única vía de autoarranque en Windows: el registro elimina el antiguo acceso directo `yt-members-signal-trader-pm2-resurrect.lnk` si existe y el script usa un bloqueo de instancia única para impedir carreras o listeners duplicados en el puerto `5178`. El arranque solo se considera correcto cuando `/api/health` devuelve `ok` y un identificador de runtime; después guarda el estado restaurable de PM2.
